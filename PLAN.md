...
- [ ] Проверка новой версии при запуске. Ловушка проверена: `/releases/latest`
      у GitHub ИСКЛЮЧАЕТ prerelease и на нашем репозитории отдаёт `Not Found`,
      потому что все релизы альфовые — брать `/releases?per_page=1`.
      Автообновления не делаем, только строка «доступна версия X».
- [ ] Локальное заглушение участника (отдельно от громкости: unmute должен
      возвращать прежнюю громкость, а не 1.0).
- [ ] Системные уведомления, когда окно скрыто. Штатный
      `pkg/services/notifications` Wails; на macOS требует валидный bundle
      identifier, то есть работает в упакованном .app и НЕ работает в
      `task dev` — нужна честная деградация, а не молчаливый отказ.
