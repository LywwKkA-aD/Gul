...
- [x] M7.1b Имена тоннеля выводятся из пароля (`relayproto.NamesFor`), релей
      принимает обе пары и логирует старую. Развёрнуто 2026-08-26; окно
      закрывается флагом `--accept-legacy-names=false`.
