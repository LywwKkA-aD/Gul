...
- [ ] Запоминать последний канал НА СЕРВЕР и заходить в него после
      подключения. Сейчас восстановление канала живёт ровно один Connect
      (manager.go: `m.restore, m.hasRestore = 0, false` при новом
      подключении), поэтому свежий вход всегда кидает в Root. Проверено по
      протоколу: «вне канала» в Mumble не существует — `UserState.channel_id`
      опционален только потому, что это дельта-сообщение, а сервер при входе
      всегда сажает в Root. Значит «выйти из канала» невозможно, и правильный
      ответ на запрос пользователя — возвращать туда, где он был. Канал,
      которого больше нет, обрабатывается как уже сделано в
      `channelToRestore` (остаёмся где посадил сервер).
