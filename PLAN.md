...
- Тема: props accent (#2F52DE | #1F6FB2 | #3E4FA8 | #5B4BD6) / density (Компакт|Средняя|Воздух → `--row-pad-y`, `--item-h`) / glow (Нет|Тонкое|Заметное → `--glow-spread`) — механика прототипа: `applyTheme()`, пишущая CSS-переменные на `document.documentElement`. **Не реализовано и вкладки «Внешний вид» в Settings НЕТ**: заглушку убрали 2026-08-24 (неактивная вкладка хуже отсутствующей). Вернуть вместе с реальным переключателем, не раньше.
...
3. **Settings (модал)** — вкладки: Звук (устройства, тест микрофона и сигнала собеседников, порог VAD, задержка отключения, громкость сигналов), Клавиши (режим передачи VAD/PTT, клавиша PTT и глобальный режим: показывать фактически назначенную комбинацию — на Wayland её выбирает композитор). «Внешний вид» и «О программе» появятся, когда за ними будет содержимое; диагностика — действие внизу боковой колонки.
...
### M4+ — бэклог (обсуждено с пользователем 2026-08-24)

- [x] Проверка новой версии при запуске (`internal/update`, `internal/core/update.go`).
      Ловушка проверена и закодирована: `/releases/latest` у GitHub ИСКЛЮЧАЕТ
      prerelease и на нашем репозитории отдаёт `Not Found`, потому что все
      релизы альфовые — берём `/releases?per_page=1`. Сравнение semver с
      prerelease-порядком — своя чистая функция с таблицей тестов.
      Автообновления не делаем, только строка «доступна версия X» со ссылкой
      на страницу релиза; отказ по версии помнится в `config.json`
      (`update.dismissed_version`). Любая ошибка — молчание.
- [x] Локальное заглушение участника (`internal/audio/users.go`): mute — своё
      состояние рядом с per-user gain, по тому же хэшу сертификата; unmute
      возвращает выбранную громкость, а не 1.0. Ноль аллокаций в RX-тике
      запинен тестом. НЕ персистится — как и per-user volume; если однажды
      начнём хранить, то оба сразу и по серверам.
- [x] Системные уведомления, когда окно скрыто (`internal/notify`,
      `internal/core/notify.go`, `notifier.go`). Ловушка проверена и обойдена:
      сервис Wails на macOS падает на старте без bundle identifier, а сервис
      в `application.Options.Services` своей ошибкой роняет ВСЁ приложение —
      поэтому он поднимается вручную, а неудача пишется в лог один раз на
      WARN. Уведомляем о сообщении в нашем канале и о входе в него, только
      когда окно не в фокусе, с лимитом (3 подряд, затем одно в 20 с).
- Факт протокола (проверено 2026-08-24, чтобы не возвращаться): «вне канала»
  в Mumble не существует — пользователь всегда ровно в одном канале, а
  `UserState.channel_id` необязателен лишь потому, что это дельта-сообщение.
  При входе сервер сажает в Root. Значит «выйти из канала» реализовать нельзя.
  Вход всегда в Root пользователь признал нормальным (2026-08-24), поэтому
  запоминание последнего канала на сервер НЕ делаем — восстановление после
  обрыва связи, которое уже есть, остаётся как есть.
- [ ] Создание и редактирование каналов + роли (кто что может). Отложено
      осознанно: без ролей создание каналов на общем сервере даёт любому
      участнику право менять дерево. Mumble уже имеет ACL и группы —
      проектировать поверх них, а не заводить своё.
