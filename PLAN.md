...
- [ ] M7.3a Память о дороге переживает перезапуск: сейчас она живёт в Manager,
      то есть один запуск. Тому, у кого WSS заблокирован, это стоит 12 секунд
      калитки на каждый старт. Нужно протащить выбор в config.json рядом с
      сохранёнными серверами и показывать выбранную дорогу в диагностике.
