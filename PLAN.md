...
└──────┼───────────────────────────────────────────────────────────────────────┘
  ...
  либо напрямую host:64738, либо внутри wss://host:443 → gul-relay (наш
...
- Подключение: адрес, ник, пароль (опц.), TLS. Две формы адреса (internal/mumble/endpoint.go): прямая `host:64738` и релейная `wss://host` — без пути: где отвечает тоннель, выводится из пароля (`relayproto.NamesFor`).
...
### M7 — Транспорт: работает из любой сети (приоритет выше M5 и M6)

Основание — живой разбор 2026-08-26. Участник из России подключался пятьдесят
раз подряд, каждый раз успешно авторизовался и слышал всех, но релей во всех
сессиях насчитал от него одни и те же ~3.3 КБ независимо от длительности:
внутреннее TLS-рукопожатие и логин, дальше ничего, даже пятисекундных пингов.
Сервер выбрасывал его по своему ping-дедлайну (murmur `timeout=30`) примерно
раз в двадцать секунд. Вывод, который и определяет весь милстоун: **успешное
рукопожатие не доказывает ничего**, годен только транспорт, по которому
подтверждён обратный ход наших пакетов. Условие пользователя (2026-08-26):
никаких внешних VPN и прокси — всё внутри клиента и сервера.

- [x] M7.1 Калитка живости: сессия принимается, только когда пришёл ответ на
      наш ping. Считать нечем другим — gumble увеличивает `tcpPacketsReceived`
      исключительно в `handlePing`, а murmur отвечает на ping только если его
      получил. Не подтвердилась за `roundTripGrace` — сессия закрывается с
      честной причиной, а не висит до серверного таймаута. Плюс дедлайн на
      запись и отдельная причина «исходящий трафик не проходит» (packetconn.go).
- [x] M7.2 QUIC-транспорт (клиент + релей): UDP/443, ALPN h3, один
      двунаправленный поток = тот же байтовый тоннель к murmur, тот же bearer.
      Честно: с одним надёжным потоком head-of-line blocking такой же, как в
      TCP; выигрыш — другой сетевой путь, лучшее восстановление после потерь и
      переживание смены IP. Датаграммы для голоса — отдельная работа в форке
      gumble, не здесь. Требует `quic-go` (жёсткий пин + заметка в docs/research).
- [x] M7.3 Автовыбор дороги (`internal/mumble/transport.go`): пробуем по
      очереди, оставляем ту, по которой калитка M7.1 подтвердила обратный ход.
      Отказ дозвона, при котором НИКТО не ответил, — тоже про дорогу, поэтому
      следующая пробуется сразу, без ожидания; ответ сервера (reject, отказ по
      учётке, 429) про дорогу не говорит и поиск не запускает. Прямой адрес —
      одна дорога, выбирать нечего.
      Порядок НЕ рандомизирован, хотя план это обещал: смысл был в том, что
      фиксированная последовательность попыток сама по себе примета, но с
      памятью попытка всего одна и последовательность почти не появляется.
- [x] M7.3a Память о дороге переживает перезапуск: `config.Server.Transport`
      рядом с сохранённым сервером, `Controller.PreferTransport` подсказывает
      её перед подключением, `Callbacks.OnTransport` записывает — но только
      когда калитка подтвердила обратный ход, и только если это новость.
      Подсказка остаётся подсказкой: дорога всё равно доказывает себя заново.
      В диагностику дорога попадает строкой `road proved itself`.
- [x] M7.1a Релей отвечает как обычный сайт (`internal/relay/cover.go`): один
      ответ на любой отказ, стоковые страницы nginx, `/healthz` невидим снаружи,
      прямой порт 64738 закрыт. Развёрнуто на проде 2026-08-26.
- [x] M7.1b Имена тоннеля выводятся из пароля (`relayproto.NamesFor`).
      Развёрнуто 2026-08-26. Окно совместимости закрыто 2026-08-27: фиксированной
      пары `/mumble` + `gul-mumble-v1` больше нет в коде, старый путь получает ту
      же 404, что и любой несуществующий адрес. Сборки до v0.4.0-alpha.2
      включительно не подключаются вовсе.
- [x] M7.1c Клиент притворяется браузером (`internal/mumble/browsertls.go`):
      ClientHello Chrome через uTLS, браузерные заголовки, Origin с портом.
      ОТМЕНЁН прежний пункт «включить h2 в ALPN»: Chrome анонсирует h2, а
      WebSocket поверх HTTP/2 в coder/websocket не реализован — релей остаётся
      на http/1.1 (docs/research/utls.md).
- [x] M7.4 Обфускация в стиле Salamander (`internal/relayproto/salamander.go`):
      скрэмблинг каждой датаграммы ключом из пароля, +8 байт соли на пакет.
      ПЕРЕКЛЮЧАТЕЛЬ НЕ СДЕЛАН и не нужен: он помогал бы, только если клиент
      знает, на какой стороне сервер, а альтернатива (чистый QUIC, видимый как
      HTTP/3) хуже того, что уже даёт TCP/443. Цена переключателя — лишняя
      неудачная попытка на каждый коннект ради режима, за который нет
      свидетельств. QUIC теперь всегда скремблится.
- [ ] M7.5 Маскарад HTTP/3 на релее: активная проба видит сайт, а не тоннель.
- [x] M7.7 Форма трафика на QUIC-дороге (`relayproto.Salamander`): случайный
      довесок к каждой датаграмме (сто голосовых пакетов перестают быть
      одинакового размера) и фоновые датаграммы со случайными паузами 20–90 мс
      (ритм перестаёт быть метрономом, тишина перестаёт выглядеть тишиной).
      Обе стороны: довесок симметричен по построению, фон шлют и клиент, и
      релей — иначе форма ломается только в одну сторону. Задержку не трогает
      ничего: только добавляем, никогда не придерживаем.
- [x] M7.4a Та же форма на WSS-дороге (`internal/relayproto/shaped.go`).
      Своё кадрирование внутри WebSocket-сообщения: тип, длина, длина довеска,
      и весь кадр округляется вверх до сетки в 256 байт. Голосовая запись любой
      длины попадает в первую ячейку, поэтому все уходят одним размером — не
      «дополненным», а одинаковым. Фон — кадр той же ячейки, пустой.
      Совместимость без флага на сервере: у контракта своё имя, выведенное из
      того же пароля, клиент предлагает оба (новое первым), релей отвечает
      первым, которое знает. Старый контракт снят 2026-08-27 — клиент всё ещё
      предлагает оба имени, чтобы пережить откат образа, но релей отвечает
      только на новое. Каждая сессия пишет в журнал, на каком контракте
      работает: по этой строке и было принято решение снимать.
      Названо прямо: частота не выравнивается. Пока человек говорит, кадров в
      секунду всё равно больше, чем когда молчит; чтобы стереть и это, нужен
      кадр на каждый такт независимо от наличия данных — другая и куда более
      дорогая конструкция. Размерная утечка закрыта полностью, частотная
      размыта.
- [x] M7.8 Переменный битрейт вместо постоянного (`internal/dsp/opus`).
      Замер решил спор: CBR при 40 кбит/с даёт РОВНО ОДИН размер кадра (50 Б)
      на все кадры без исключения, VBR — сорок разных от 34 до 99 при том же
      среднем. Ограничение включено, чтобы среднее держалось в цели: сервер
      считает полосу на пользователя. Цена названа: размеры кадров теперь
      следуют энергии речи. Закрыто на обеих дорогах: на QUIC датаграммы одного
      размера, на WSS — сетка кадров (M7.4a).
- [ ] M7.6 Склейка 2–3 кадров в пакет (настройка): 118 → 60–75 кбит/с и конец
      метроному 100 пакетов в секунду; цена — +10–20 мс задержки.
      «Склейка без ожидания» из шага 10 НЕ делается: она не бесплатна. Слить
      несколько кадров в один пакет на уровне Mumble нельзя — murmur такого не
      поймёт; на уровне транспорта это то же кадрирование через релей, что и
      паддинг WSS. Разброс в паузах переподключения делать не пришлось — он уже
      есть с M1 (`backoff.go`, ±20%, запинено тестами).
- Паддинг — только если замеры покажут необходимость. Замер 2026-08-26: полезных
  40 кбит/с, в проводе ~118 кбит/с (две трети — обвязка: 100 пакетов/с, двойной
  TLS, WebSocket, TCP/IP). Паддинг до 256 Б даёт ×2.4, до MTU 1200 Б — ×8.7.
  Обфускация не стоит ничего, склейка экономит; платит только паддинг.
- Не берём из Hysteria их Brutal: он придуман продавливать сотни мегабит через
  throttling, нам нужно сорок килобит, и он намеренно нечестен к соседям.
- **Готово, когда:** клиент из сети, где WSS не работает, сам находит рабочий
  транспорт и разговаривает; в диагностике видно, какой транспорт выбран и по
  какой причине отвергнуты остальные.
