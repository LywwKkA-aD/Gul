...
- [x] M7.7 Форма трафика на QUIC-дороге (`relayproto.Salamander`): случайный
      ...
      НЕ СДЕЛАНО: та же форма на WSS-дороге. Там байтовый поток идёт насквозь к
      murmur, поэтому довесок требует своего кадрирования через релей — это
      отдельная работа, и её стоит делать, только если WSS останется основной
      дорогой.
- [x] M7.8 Переменный битрейт вместо постоянного (`internal/dsp/opus`).
      ...
      следуют энергии речи. На QUIC-дороге паддинг это закрывает, на WSS —
      стоит до её паддинга.
