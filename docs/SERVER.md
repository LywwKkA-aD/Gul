# Прод-сервер Gul

Сервер — готовый `mumblevoip/mumble-server` (murmur) v1.5.915, своего кода на
сервере нет, кроме WSS-релея. Этот документ — операторская памятка: топология,
порты, секреты, бэкап, обновление, где смотреть логи. Детали каждого
компонента — в `deploy/murmur/` и `deploy/relay/README.md`.

## Топология

```text
клиент Gul ──wss://murmur.gulvox.com/mumble:443──► firewalld redirect 443→8443
                                                   └─► gul-relay (rootless podman, сетевой
                                                        namespace murmur) ──► 127.0.0.1:64738 murmur
клиент Gul / официальный Mumble ──host:64738 (tcp)──────────────────────────► murmur
```

- Релей — непрозрачный байтовый насос с фиксированной целью (loopback-murmur).
  Внутри WSS идёт полный Mumble TLS с тем же TOFU-отпечатком, что и напрямую:
  релей доверие не терминирует и пароль сервера не хранит (DECISIONS 2026-08-23).
- Прямой порт 64738 остаётся для сетей, где его не режут, и для официального
  клиента Mumble (он же использует UDP 64738 для голоса; Gul голос гонит только
  по TCP-туннелю).

## Порты

| Порт | Кто | Зачем |
|---|---|---|
| 443/tcp (публичный) | firewalld redirect → 8443 | WSS-вход клиентов Gul |
| 8443/tcp (только localhost/namespace) | gul-relay | слушатель релея; в публичной зоне не открывать |
| 64738/tcp | murmur | прямой Mumble (Gul и официальный клиент) |
| 64738/udp | murmur | голос официального клиента; Gul не использует |

Правила host-фаерволла с лимитом соединений на источник (nftables и firewalld,
IPv6 маскируется до /64) — в `deploy/relay/README.md`, раздел «Host firewall».
Это второй эшелон: у самого релея есть per-source лимит до TLS, но на
пользовательском уровне connlimit дешевле и срабатывает раньше.

## Секреты

- `MUMBLE_SUPERUSER_PASSWORD` — задаётся явно при первом запуске murmur
  (иначе он сгенерирует свой и утопит в логах). Нужен только для админских
  действий через официальный клиент (регистрация, ACL, каналы).
- `MUMBLE_CONFIG_SERVER_PASSWORD` — пароль входа на сервер; его знают все
  участники. Из него клиент выводит bearer релея.
- `GUL_RELAY_BEARER` — podman-секрет с предвычисленной bearer-строкой
  (`gul-relay derive-credential`, одна строка с префиксом `v2.`). Релей
  читает только его; сырой пароль в контейнер релея не монтируется.
  Релей без `v2.`-строки не стартует; строка без префикса игнорируется.
  Процедура — в README релея.

Ротация пароля сервера = ротация `GUL_RELAY_BEARER` (они связаны по построению).

## Данные и бэкап

Всё состояние murmur — том `/data` (владелец UID/GID 10000:10000): SQLite-база
(регистрации, каналы, ACL, баны), сертификат сервера (`mumble.crt`/`mumble.key` —
это и есть TOFU-идентичность, которую пинят клиенты) и подкаталог `acme`
с цепочкой для публичного имени (её же читает релей, только чтение).

Бэкап: `podman volume export` тома (или tar из `/data` при остановленном
murmur) по расписанию, хранить минимум две копии вне хоста. Потеря
`mumble.key` = смена отпечатка у всех клиентов (TOFU-предупреждение и
переподтверждение у каждого) — это самый дорогой файл на сервере.

## Незаметность: релей отвечает как обычный сайт

Сделано 2026-08-26. До этого хост опознавался одним запросом: `GET /mumble`
отдавал `WWW-Authenticate: Bearer realm="gul-relay"`, а корень — голый
`404 page not found` без заголовка `Server`. Различимых ответов было десять,
и по ним читалось всё дерево решений сервиса.

Теперь у хоста одна личность (`internal/relay/cover.go`):

- корень отдаёт страницу стокового nginx (200, `Server: nginx`, `ETag`,
  `Last-Modified`);
- **любой** отказ — не тот путь, не тот метод, не тот хост, нет ключа, не тот
  ключ, нет подпротокола — отдаёт байт в байт ту же страницу 404, что и
  несуществующий адрес. Причина уходит в лог, а не в ответ;
- `/healthz` вне loopback неотличим от несуществующего адреса;
- 429 и 503 сохранены: занятый сайт так и отвечает, а клиент по ним ждёт.

Свои страницы подставляются через `Config.ServerHeader`, `CoverIndex`,
`CoverNotFound`.

Следствие для клиента: неверный пароль теперь неотличим от неверного адреса —
это и есть цель. Клиент говорит «неверный адрес сервера или пароль».

h2 пока НЕ анонсируется. Если добавить его в ALPN, существующие клиенты
договорятся на HTTP/2 и WebSocket-апгрейд сломается: это идёт вместе с
браузерным транспортом в M7.5.

### Прямой порт Mumble закрыт снаружи

Открытый 64738 сообщал сканеру, что за трафик идёт на 443 — то самое, что
релей и прячет. За семь дней логов на этом порту были только сканеры (пробы
разными версиями TLS, HTTP-запросы на Mumble-порт); все реальные сессии идут
через релей, а он ходит к murmur по loopback внутри своего namespace и
правилом не задет.

Правило — в `/etc/nftables/gul-relay-guard.nft`, рядом с connlimit:
`iif != "lo" tcp dport 64738 counter drop` и то же для udp. Откат: убрать две
строки и `nft -f /etc/nftables/gul-relay-guard.nft`, либо вернуть
`.bak-20260826-cover`.

## Второй путь: QUIC на UDP 443

Живо с 2026-08-27. Релей слушает тот же адрес по UDP и принимает тот же
тоннель с той же учёткой (`internal/relay/quic.go`); клиент умеет обе дороги и
пока переключается на QUIC при недозвоне по WebSocket.

Что для этого понадобилось на сервере, по порядку — каждый пункт с откатом:

1. **Публикация порта.** Релей делит сетевой namespace с murmur, а порты
   публикует `gul-murmur.container`. Добавлено
   `PublishPort=0.0.0.0:8443:8443/udp` рядом с tcp-строкой. Требует рестарта
   `gul-murmur.service`, то есть обрывает активные сессии. Откат — файл
   `.bak-20260827-quic` рядом.
2. **Редирект.** `firewall-cmd --permanent --add-forward-port=port=443:proto=udp:toport=8443`
   плюс `--reload`. Проверено, что наша таблица `inet gul_relay_guard`
   перезагрузку firewalld переживает (она отдельная и с этим и заводилась).
   Откат — `--remove-forward-port` с теми же параметрами.
   ЗАМЕЧАНИЕ: редирект, как и tcp-шный, только IPv4 — прежний пробел про
   IPv6-клиентов остаётся и на QUIC.
3. **Буфер UDP.** quic-go просит 7 МиБ на приём и на каждом старте ругается в
   лог, если лимит меньше. `/etc/sysctl.d/99-gul-quic.conf` поднимает
   `net.core.rmem_max`/`wmem_max` до 7.5 МБ. Голосу такая полоса не нужна;
   это про тишину в логе. Откат — удалить файл и `sysctl --system`.

### QUIC всегда скремблится

С 2026-08-27 каждая датаграмма скремблится ключом из пароля
(`internal/relayproto/salamander.go`). Снаружи опознавать нечего: ни версии, ни
SNI, ни ALPN, ни самого факта, что это QUIC. Проверено на проде — и с неверным
паролем, и обычным QUIC-сканером порт молчит вовсе (таймаут, ноль ответов), а с
верным паролем murmur отвечает через тоннель.

Переключателя «скремблить или нет» НЕТ и не планируется — почему, в
docs/DECISIONS.md за 2026-08-27.

Проверка положительного пути без выноса секрета: собрать маленький пробник,
запустить его НА сервере и дать ему путь к учётке внутри namespace релея
(`/proc/$(pgrep -f 'gul-relay --listen')/root/run/secrets/GUL_RELAY_BEARER`) и
цель `127.0.0.1:8443`. Секрет при этом не покидает машину.

Выключается флагом `--quic=false` в `Exec=`. Неудача при открытии UDP-слушателя
НЕ фатальна: релей, отвечающий по TCP, остаётся рабочим релеем.

Проверка снаружи: подключиться клиентом с заведомо неверным паролем по QUIC —
в журнале появится `relay quic authorization rejected` с `credential=v2`. Это
доказывает весь путь: редирект, публикацию, слушатель, преамбулу и проверку.

## Обновление релея: пошагово

Проверено на проде 2026-08-24 (переход alpha.2 → hardened). Всё выполняется
под пользователем `murmur`; `XDG_RUNTIME_DIR=/run/user/1001` обязателен, иначе
podman не найдёт сокет. Заходить лучше через `runuser -u murmur --`, при этом
`podman exec` и `podman healthcheck run` вручную падают на cgroup (нет
systemd-сессии) — это не поломка, штатные проверки контейнера идут своим
путём и возвращают 0.

1. Собрать статический бинарь и лицензионный бандл на рабочей машине:
   `CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags="-s -w -X main.version=$(bash scripts/check-version.sh --print app)" -o gul-relay ./cmd/gul-relay`
   и `bash scripts/collect-licenses.sh legal`.
2. Перенести `gul-relay`, `legal/` и `deploy/relay/Containerfile` на сервер,
   собрать образ: `podman build -t gul-wss-relay:<tag> -f Containerfile .`
3. Взять digest: `podman inspect --format "{{.Digest}}" localhost/gul-wss-relay:<tag>`.
4. **Пересоздать секрет** — только если меняется пароль сервера. Значение не
   должно попадать в терминал:
   ```sh
   podman run --rm \
     --secret MUMBLE_CONFIG_SERVER_PASSWORD,type=mount,target=MUMBLE_CONFIG_SERVER_PASSWORD,uid=10000,gid=10000,mode=0400 \
     --entrypoint /usr/local/bin/gul-relay localhost/gul-wss-relay:<tag> \
     derive-credential --secret-file /run/secrets/MUMBLE_CONFIG_SERVER_PASSWORD > /tmp/cred.txt
   podman secret rm GUL_RELAY_BEARER; podman secret create GUL_RELAY_BEARER /tmp/cred.txt; shred -u /tmp/cred.txt
   ```
   Проверка без раскрытия значения: `wc -l` = 1 и `head -c3` = `v2.`.
5. В `~/.config/containers/systemd/gul-relay.container` заменить `Image=` на
   новый digest. Старый файл сохранить рядом как `.bak-<дата>` — это и есть
   путь отката (вернуть файл, `daemon-reload`, `restart`); прежний образ
   остаётся на хосте, `Pull=never` его не тронет.
6. `systemctl --user daemon-reload && systemctl --user restart gul-relay.service`
7. Проверить: `podman ps` показывает `(healthy)`, а в журнале появляется
   `relay ready`. Подключиться клиентом; в журнале должны быть
   `relay session opened` / `closed` с байтами, транспортом и причиной.

**Состояние на 2026-08-27: оба окна совместимости закрыты.** Секрет — одна
строка `v2.`; фиксированной пары `/mumble` + `gul-mumble-v1` больше нет в коде,
как и флагов `--accept-legacy-bearer` / `--accept-legacy-names`. Клиенты
v0.4.0-alpha.2 и старше не подключаются: старый путь отдаёт ту же 404, что и
любой несуществующий адрес, а старый ключ — `relay authorization rejected` с
`credential=legacy`.

Выкачено образом `sha256:b00dda65…8432e8`, прежний — `sha256:e83833ee…d059db`,
он остаётся на хосте; юнит сохранён как `.bak-20260827-nolegacy`. Откат: вернуть
файл, `daemon-reload`, `restart` — прежний `Exec=` содержит
`--accept-legacy-bearer=false`, которого новый бинарь уже не понимает, поэтому
образ и юнит откатываются только вместе.

Проверено на проде после выката: выведенный путь — 101; `/mumble` с верным
ключом — 404, байт в байт та же страница, что и несуществующий адрес (139
байт); QUIC — murmur ответил своим TLS (CN `murmur.gulvox.com`), а с неверным
паролем порт молчит до таймаута.

## Форма на WSS: два контракта, выбирает клиент

Живо с 2026-08-27, образ `sha256:5c9a9b2b…fa4820` (прежний —
`sha256:b00dda65…8432e8`, юнит сохранён как `.bak-20260827-shaped`).

У тоннеля два имени подпротокола, оба выведены из пароля. Новое кадрирует
каждую запись и округляет кадр до сетки в 256 байт, так что длины перестают
следовать кодеку (`internal/relayproto/shaped.go`); старое — обычный байтовый
поток. Клиент предлагает оба, новое первым, релей отвечает первым, которое
знает. **Флага на сервере нет и не будет** — окно закрывается удалением
старого имени, когда в журнале перестанут появляться сессии с ним.

Проверка положительного пути, как и у QUIC: собрать пробник, запустить его НА
сервере, дать путь к учётке внутри namespace релея и цель `127.0.0.1:8443`.
Проверено обоими способами: клиент, предлагающий оба имени, получает новый
контракт и доходит до murmur; клиент, знающий только старое имя, получает
откат и тоже доходит. Значит сборки 0.5.0 продолжают работать.

**Кто на каком контракте — с 2026-08-27 видно в журнале**, образ
`sha256:42a84cce…c98bbc`. Каждая сессия пишет `contract`: `plain` или `shaped`.
До этого поля не было, и утверждение «по журналу видно, кто ещё на старом» не
соответствовало ни одному журналу, который у нас был.

```sh
journalctl CONTAINER_NAME=gul-wss-relay --since "7 days ago" | grep '"contract":"plain"'
```

Сами имена в журнал не попадают: они выведены из пароля.

**Старый контракт снят 2026-08-27**, образ `sha256:d1883c11…3469a8a` (прежний —
`sha256:42a84cce…c98bbc`, юнит сохранён как `.bak-20260827-shapedonly`). Релей
отвечает только на новое имя. Клиент 0.5.1 предлагает оба, поэтому откат образа
его не ломает; клиент 0.5.0 знает только старое имя и **не подключается**.

Проверено на проде после выката: клиент, предлагающий оба, доходит до murmur и
пишется в журнал как `contract:"shaped"`; клиент, знающий только старое имя,
получает 404 — байт в байт ту же страницу (139 байт), что и адрес, которого
никогда не было. Это и есть смысл: закрытие не должно само стать приметой.

Следствие для пользователя: сборка 0.5.0 говорит «неверный адрес сервера или
пароль», а не «обнови клиент». Понятнее сделать нельзя, не сломав маскировку —
все отказы обязаны выглядеть одинаково.

### Кадр стал ровно одной ячейкой — 2026-08-27

Образ `sha256:899e24d9…7c1c81` (прежний — `sha256:baa2011c…c61240`, юнит
сохранён как `.bak-20260827-cells`). Откат: вернуть файл, `daemon-reload`,
`restart`; прежний образ на хосте, `Pull=never` его не тронет. Здесь откат
безопаснее обычного — читатель разбирает кадры любого размера, так что старый
релей и новый клиент понимают друг друга в обе стороны.

Округление до сетки пряталo голос и не прятало рукопожатие: записи внутреннего
TLS по 1300–1500 байт превращались в кадры 1536 и 1280 — единственные два
размера за сессию, не равные 256. Теперь `Write` режет по ячейке, и на проводе
один размер с первого байта (`docs/DECISIONS.md`, замер).

**Почему это надо было выкатить симметрично.** Шейпер общий для обеих сторон
(`relayproto.Shape`), поэтому до выката релей продолжал выкладывать вниз кадры
1536/1280, и проверка «помогла ли ровная сетка» была бы сделана наполовину.

Проверено после выката: `relay ready` с версией `0.5.2-alpha.7`, живые сессии
переподключились и пишутся как `contract:"shaped"`.

### Прикрытие и общий допуск — 2026-08-27

Образ `sha256:75909179…f6981b` (прежний — `sha256:899e24d9…7c1c81`, юнит
сохранён как `.bak-20260827-cover`).

Что изменилось снаружи: заголовок `ETag` уходит под тем написанием, которое
шлют все остальные серверы. До выката прод отдавал `Etag` — канонизация Go, —
и это опознавало хост как Go-программу одним запросом. Проверено после выката:

```sh
curl -sI https://murmur.gulvox.com/   # ETag: "65e5928a-1ca"
```

Отказы 429 и 503 теперь отдают ту же страницу nginx, что и 404, вместо
`text/plain` от `http.Error`. Метка контракта на дороге QUIC перестала врать:
`padded` вместо `shaped`. Адрес Murmur стал флагом `--upstream`.

Что осталось известным пределом: **порядок заголовков**. `net/http` пишет
пользовательские отсортированными и добавляет `Date` после них, nginx начинает с
`Server` и `Date`. Управлять порядком в Go нельзя без перехвата соединения.
Там же отсутствуют `Connection: keep-alive` и `Accept-Ranges`, а ALPN предлагает
только `http/1.1`.

**Эта ложь исправлена 2026-08-27** тем же образом: дороги теперь пишут разные
метки — `shaped` и `padded`, — и тест держит их врозь. На QUIC маскировки
размеров по-прежнему нет; разница в том, что журнал больше не утверждает
обратное.

## Обновление

- Образ murmur запинен тегом `v1.5.915` (PLAN §2); ветка 1.6 — RC, не брать.
- Релей: собрать Linux amd64 с `CGO_ENABLED=0` (CI делает это статически и
  проверяет), собрать scratch-образ из `deploy/relay`, подставить digest в
  quadlet (`Pull=never`, образ только локальный), `systemctl --user daemon-reload`
  и перезапуск юнита. Quadlet объявляет `PartOf=gul-murmur.service`: рестарт
  murmur перезапускает релей; релей дренирует сессии close-фреймом до 5 с,
  после чего клиенты переподключаются сами.
- Порядок при несовместимых изменениях протокола релея: сначала релей,
  принимающий обе формы, затем клиенты, затем удаление старой формы. Оба
  прошлых окна (bearer v1, фиксированные имена) прошли этот путь и закрыты.

## Ограничение соединений на фаерволе

Развёрнуто 2026-08-24 и проверено нагрузкой. У релея есть собственный лимит на
источник, но это внешний, более дешёвый рубеж: флуд гасится ядром до того, как
стоить TLS-рукопожатия. Порог 32 новых соединения с адреса (IPv4 /32, IPv6 /64)
против 16 у самого релея, так что обычная работа его не достаёт.

Файлы: `/etc/nftables/gul-relay-guard.nft` (правила) и
`/etc/systemd/system/gul-relay-guard.service` (загрузка при старте, `enabled`).

Размещение правила — несущая деталь. firewalld редиректит 443 → 8443 в nat
prerouting с приоритетом −100, поэтому правило после него увидит уже 8443, а не
443; conntrack работает на −200, и `ct state`/`ct count` без него недоступны.
Приоритет **−110** — единственное окно, где виден публичный порт И уже есть
conntrack. Действие `drop`, а не `reject`: в prerouting reject недоступен, да и
флуд не должен оплачиваться ответным пакетом на каждый отброшенный SYN.

Таблица своя (`inet gul_relay_guard`), поэтому firewalld — владелец таблицы
`inet firewalld` — её не трогает, и `firewall-cmd --reload` её не сотрёт.

Проверка (45 одновременных соединений с одного адреса): установилось 32,
остальные отброшены, счётчик правила вырос. Текущее состояние:

```sh
nft list table inet gul_relay_guard   # счётчики packets/bytes у обоих правил
systemctl status gul-relay-guard
```

Известное ограничение: редирект 443 → 8443 у firewalld объявлен только для
IPv4 (`meta nfproto ipv4`), поэтому IPv6-клиенты до релея пока не доходят —
правило relay6 написано на будущее и сейчас всегда нулевое.

## Наблюдаемость

- Релей пишет JSON-логи в journald (`journalctl --user -u gul-relay`):
  открытие/закрытие сессий с байтами и причиной, 401 с классом credential,
  активация бана, отказ по ёмкости, недоступность murmur, ошибки перезагрузки
  сертификата. Таблица «класс отказа → что видно в логе» — в README релея.
- `gul-relay healthcheck` (HealthCmd quadlet) проверяет TLS и `/healthz`
  (доступен только с loopback); при проблемах с перезагрузкой сертификата
  в теле ответа появляется `certificate-reload: failing`, но юнит не убивается.
- murmur: `journalctl --user -u gul-murmur`; autoban в проде **включён**
  (в дев-стенде выключен ради smoke-тестов). Релей подставляет источникам
  псевдонимные loopback-адреса (по /32 или /64), так что автобан murmur
  продолжает работать по блокам источников, а не по одному адресу релея.

## Дев-стенд

`task murmur:up` поднимает тот же образ локально на 127.0.0.1:64738 с
выключенным autoban; для LAN — переменные `MUMBLE_BIND_ADDRESS` и
`MUMBLE_SUPERUSER_PASSWORD` (README проекта).
