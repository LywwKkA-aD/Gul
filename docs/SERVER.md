...
**Известная ложь в журнале:** `internal/relay/quic.go` подписывает QUIC-сессии
как `contract:"shaped"`, хотя `relayproto.Shape` на дороге QUIC не вызывается
ни на одном конце — вызовы только в `internal/relay/handler.go` и
`internal/mumble/wss.go`. То есть на QUIC маскировки размеров нет вовсе, а
журнал утверждает обратное. Не чинить это молча: по этой метке читались
журналы при разборе обрыва у пользователей.
